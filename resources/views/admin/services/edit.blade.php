...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.services.update', [$service->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($categories as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('category_id') ? old('category_id') : $service->category->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                        name="name" id="name" value="{{ old('name', $service->name) }}" required>
                    @if ($errors->has('name'))
                        <div class="invalid-feedback">
                            {{ $errors->first('name') }}
                        </div>
                ...
                <div class="form-group">
                    <label for="inside_image">{{ trans('cruds.service.fields.inside_image') }}</label>
            ...
            <div class="form-group">
                <label for="small_image">{{ trans('cruds.service.fields.small_image') }}</label>
                <div class="needsclick dropzone {{ $errors->has('small_image') ? 'is-invalid' : '' }}" id="small_image-dropzone">
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.service.fields.small_image_helper') }}</span>
            </div> --}}
...
@section('scripts')
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('admin.services.storeCKEditorImages') }}',
                                            true);
    ...
    <script>
        Dropzone.options.insideImageDropzone = {
            ...
            init: function() {
                @if (isset($service) && $service->inside_image)
                    var file = {!! json_encode($service->inside_image) !!}
                    this.options.addedfile.call(this, file)
                    this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
                    file.previewElement.classList.add('dz-complete')
    ...
    <script>
        Dropzone.options.smallImageDropzone = {
            ...
            init: function() {
                @if (isset($service) && $service->small_image)
                    var file = {!! json_encode($service->small_image) !!}
                    this.options.addedfile.call(this, file)
                    this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
                    file.previewElement.classList.add('dz-complete')
